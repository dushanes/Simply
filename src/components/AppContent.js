...
export const AppContent = ({ID, recipe, useMetric, ingred, instructions, popularData, getPopularData, exploreData, getExploreData, onSearch, onSelectChange}) => {
    ...
    */
    ...
    return (ID < 0) ?
        <div className='content-container' style={{flexDirection: 'row'}}>
            <HomeMostPopular 
                data={popularData} 
                getData={getPopularData} 
                onSelectChange={onSelectChange} 
                onSearch={onSearch}/>
            <HomeExplore 
                data={exploreData} 
                getData={getExploreData} 
                onSelectChange={onSelectChange}
                onSearch={onSearch}/>
        </div>
        :
